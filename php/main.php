 <?php
    ...
    if($action=="update_status"){
       $query="UPDATE an74.task SET Status = '$status' WHERE taskID = '$taskid'";
       updateQuery($query,$conn);
       header("location: ../main.php");
    }

    if($action=="delete_task"){
       $query="DELETE FROM an74.task WHERE taskID = '$taskid'";
       updateQuery($query,$conn);
       header("location: main.php");
    }
    ...
    $query = "SELECT *FROM an74.task where Username = '$login_session' and Status=0 ORDER BY DueDate ".$order1;
    $todos= runQuery($query,$conn);

    //Send the query to show all completed task tasks
    $query = "SELECT *FROM an74.task where Username = '$login_session' and Status=1 ORDER BY DueDate ".$order2;
    $completed= runQuery($query,$conn);


    //Send the query to show all very important level task tasks
    $query = "SELECT *FROM an74.task where Username = '$login_session' and Status=0 AND Urgency='Very important'";
